...
<div class="m_container w-container">
    <div class="complete_courses_list_field w-row">
        <div class="w-col w-col-8 w-col-stack">
          ...
          @endif
            <h1 class="normal_title"><strong class="accent">OWNED</strong> COURSES.</h1>
            <div class="w-layout-grid grid">
                @forelse ($courses as $course)
                <a id="w-node-f8f2acd29a99-cd1814dc" style="
                ...
                " href="" class="course_in_list w-inline-block">
                    <div class="course_thumbnail">
                        <div class="dark_overlay">
                            <div class="featured_course_title">{{ $course->getCourse->title }}</div>
                        </div>

                        <img src="{{ asset('uploads/course') }}/{{ $course->getCourse->cover_image }}" loading="lazy" alt="" class="image">
                    </div>
                    ...
                    @else
                      @if($course->status == 'free')
                        @auth
                          <div class="text-block-3"><strong class="bold-text-3">Owned Course</strong></div>
                        @else
                          <div class="text-block-3">€<strong class="bold-text-3">{{ $course->getCourse->price }}</strong>,-</div>
                        @endauth
                      @else
                        <div class="text-block-3">€<strong class="bold-text-3">{{ $course->getCourse->price }}</strong>,-</div>
                      @endif
                    ...
                    <div class="course_info in_list"><strong class="detail">Lessons:</strong> <strong
                            class="detail_data">{{ $course->getCourse->getLessons->count() + 1 }}</strong></div>
                    <div class="course_info in_list"><strong class="detail">Category:</strong> <strong
                            class="detail_data">{{ $course->getCourse->getCategory->category_name }}</strong></div>
                    <div class="course_info in_list"><strong class="detail">Total Length:</strong> <strong
                            class="detail_data">{{ $course->getCourse->course_length }}</strong></div>
                    <div class="course_info in_list"><strong class="detail">Status:</strong> <strong
                            class="detail_data">{{ $course->getCourse->status }}</strong></div>
                </a>
                @empty
                    <h5 style="color: #fff;">You did not buy any course but you can check for free courses below.</h5>
                @endforelse
            </div>
            <div class="w-layout-grid grid">
            ...
            <div class="course_details">
                ...
                <div class="w-form">
                    <form action="{{ route('front.search') }}" method="get">
                        ...
                        <input style="margin-left : 0px !important;" type="submit" class="small_button w-button" value="SEARCH">
                        </form>
                </div>
